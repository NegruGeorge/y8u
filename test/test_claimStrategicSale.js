...
describe("Y8uDistributor Tests StategicSale", function () {
    ...
    beforeEach(async () => {
        ...
        allocations = [
            ...
            [addrOverAllocated.address, ethers.parseEther("5001234")],
        ];
    ...
    async function claimTokens(claimer, totalAllocation, account) {
        ...
        await distributor.connect(claimer).claimStrategicSale(totalAllocation, proof);
    }
    ...
    it("Should adjust total claimed amount after a successful claim", async function() {
      ...
      const totalClaimedAfter = await distributor.totalClaimedStrategicSale();
      expect(totalClaimedAfter).to.equal(initialAllocation / BigInt(10));
  });
  ...
  it("Should prevent claiming with a proof for a different account", async function() {
      ...
      await expect(claimTokens(addr1, allocation, addr2)).to.be.revertedWith("Invalid Merkle proof Strategic sale");
  });
  ...
  it("Total supply should not exceed max supply limit after claims", async function() {
      ...
      expect(await distributor.totalClaimedStrategicSale()).to.eq(ethers.parseEther("105000") / BigInt(10))
  });
