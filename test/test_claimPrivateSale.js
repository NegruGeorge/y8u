...
describe("Y8uDistributor Tests ", function () {
    ...
    beforeEach(async () => {
        ...
        allocations = [
            ...
            [addr5.address, ethers.parseEther("19894800")],
        ...
        expect(await distributor.calculateClaimablePrivateSale(ethers.parseEther("100000"), addr1)).to.eq(0)
        
        await distributor.setTgeTimestamp();
        await expect(claimTokens(addr1, ethers.parseEther("100000"), addr1)).to.be.revertedWith("Invalid Merkle proof Private sale")
    ...
    it("Should allow first valid claim in first month", async function () {
        ...
        expect(balance).to.equal(allocation * BigInt(200_000) / BigInt(20_000_000));
    });
    ...
    it("Should allow first valid claim in second month", async function () {
        ...
        const balance = await token.balanceOf(addr1.address);
        expect(balance).to.equal(allocation * BigInt(400_000) / BigInt(20_000_000));
    });
    ...
    it("Should allow first valid claim in 3rd month", async function () {
        ...
        const balance = await token.balanceOf(addr1.address);
        expect(balance).to.equal(allocation * BigInt(600_000) / BigInt(20_000_000));
    });
    ...
    it("Should allow first valid claim in 4rd month", async function () {
        ...
        const balance = await token.balanceOf(addr1.address);
        expect(balance).to.equal(allocation * BigInt(600_000) / BigInt(20_000_000) 
            + allocation * BigInt(970_000) / BigInt(20_000_000));
    });
    ...
    it("Should allow first valid claim in 10th month", async function () {
        ...
        const balance = await token.balanceOf(addr1.address);
        expect(balance).to.equal(allocation * BigInt(600_000) / BigInt(20_000_000) 
            + BigInt(7) * allocation * BigInt(970_000) / BigInt(20_000_000));
    });
    ...
    it("Should allow first valid claim in 23th month", async function () {
        ...
        const balance = await token.balanceOf(addr1.address);
        expect(balance).to.equal(allocation * BigInt(600_000) / BigInt(20_000_000) 
            + BigInt(20) * allocation * BigInt(970_000) / BigInt(20_000_000));
    });
    ...
    it("Should claim every month to test if user gets it right ", async function () {
        ...
        balance = await token.balanceOf(addr1.address);
        expect(balance).to.equal(allocation * BigInt(600_000) / BigInt(20_000_000) 
        + BigInt(2) * allocation * BigInt(970_000) / BigInt(20_000_000));

        await increaseTime(1);
        await claimTokens(addr1, allocation, addr1);
        balance = await token.balanceOf(addr1.address);
        expect(balance).to.equal(allocation * BigInt(600_000) / BigInt(20_000_000) 
        + BigInt(3) * allocation * BigInt(970_000) / BigInt(20_000_000));
   
        await increaseTime(1);
        await claimTokens(addr1, allocation, addr1);
        balance = await token.balanceOf(addr1.address);
        expect(balance).to.equal(allocation * BigInt(600_000) / BigInt(20_000_000) 
        + BigInt(4) * allocation * BigInt(970_000) / BigInt(20_000_000));
        ...
        balance = await token.balanceOf(addr1.address);
        expect(balance).to.equal(allocation * BigInt(600_000) / BigInt(20_000_000) 
        + BigInt(19) * allocation * BigInt(970_000) / BigInt(20_000_000));
    ...
    it("Should claim random to test if user gets it right ", async function () {
        ...
        balance = await token.balanceOf(addr1.address);
        expect(balance).to.equal(allocation * BigInt(600_000) / BigInt(20_000_000) 
        + BigInt(14) * allocation * BigInt(970_000) / BigInt(20_000_000));
    ...
    it("Should over allocate", async function () {
        ...
        await expect(claimTokens(addrOverAllocated, ethers.parseEther("1234"), addrOverAllocated)).to.be.revertedWith("Private sale allocation is 100%");
    ...
    it("Should claim random to test if user gets it right ", async function () {
        ...
        expect(balance).to.equal(ethers.parseEther("5") * BigInt(600_000) / BigInt(20_000_000) 
        + ethers.parseEther("5") * BigInt(970_000) / BigInt(20_000_000));
        ...
        await claimTokens(addr1, ethers.parseEther("100000"), addr1);
        balance = await token.balanceOf(addr1.address);
        expect(balance).to.equal(ethers.parseEther("100000") * BigInt(600_000) / BigInt(20_000_000) 
        + BigInt(4) * ethers.parseEther("100000") * BigInt(970_000) / BigInt(20_000_000));

        await claimTokens(addr3,  ethers.parseEther("5"), addr3);
        balance = await token.balanceOf(addr3.address);
        expect(balance).to.equal(ethers.parseEther("5") * BigInt(600_000) / BigInt(20_000_000) 
        + BigInt(4) *  ethers.parseEther("5") * BigInt(970_000) / BigInt(20_000_000));

        await claimTokens(addr5,  ethers.parseEther("19894800"), addr5);
        balance = await token.balanceOf(addr5.address);
        expect(balance).to.equal( ethers.parseEther("19894800") * BigInt(600_000) / BigInt(20_000_000) 
        + BigInt(4) *   ethers.parseEther("19894800") * BigInt(970_000) / BigInt(20_000_000));
        ...
        expect(balance).to.equal(ethers.parseEther("5000") * BigInt(600_000) / BigInt(20_000_000) 
            + BigInt(20) * ethers.parseEther("5000") * BigInt(970_000) / BigInt(20_000_000));
        ...
        expect(balance).to.equal(ethers.parseEther("195")* BigInt(600_000) / BigInt(20_000_000) 
            + BigInt(20) * ethers.parseEther("195") * BigInt(970_000) / BigInt(20_000_000));
        expect(balance).to.equal(ethers.parseEther("195"));    

        await claimTokens(addr5, ethers.parseEther("19894800"), addr5);
        balance = await token.balanceOf(addr5.address);
        expect(balance).to.equal(ethers.parseEther("19894800")* BigInt(600_000) / BigInt(20_000_000) 
            + BigInt(20) * ethers.parseEther("19894800") * BigInt(970_000) / BigInt(20_000_000));
        expect(balance).to.equal(ethers.parseEther("19894800"));  
        ...
        expect(await distributor.totalClaimedPrivateSale()).to.eq(await distributor.PRIVATE_SALE());
    ...
    it("Should adjust total claimed amount after a successful claim", async function() {
      ...
      const totalClaimedAfter = await distributor.totalClaimedPrivateSale();
  ...
  it("Total supply should not exceed max supply limit after claims", async function() {
      ...
      expect(await distributor.totalClaimedPrivateSale()).to.eq(ethers.parseEther("105000") / BigInt(100))
  });
